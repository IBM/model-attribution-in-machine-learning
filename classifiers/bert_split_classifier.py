...
class BertClassifier(nn.Module):
    ...
    def save_model(self, base_model):
        self.bert.save_pretrained(f'./files/split_bert_{base_model}_classifier')
        torch.save(self.linear.state_dict(), f'./files/split_bert_{base_model}_classifier/ft_layer.pt')
        torch.save(self.dropout.state_dict(), f'./files/split_bert_{base_model}_classifier/dropout_layer.pt')

    def load_model(self, base_model):
        self.bert.from_pretrained(f'./files/split__{base_model}_classifier')
...
for base_model in base_model_to_training_ft.keys():
                ...
                if len(model_name) < 3:
                    if model_name in training_ft_models.keys():
                        if gt_ft_model == model_name:
                            ...
                            labels.append(0)
                        data_model_name.append(model_name)
                        prompt_responses.append(response)#prompts[prompt][base_model]) + '[SEP]' + response)
    ...
    torch.save(model.state_dict(), f'./files/split_bert_{base_model}_classifier/model.pth')
    correct_predictions = 0
    model_correctness = {str(i): 0 for i in range(len(testing_ft_models))}
    model_correctness[base_model] = 0
    model_predictions = {str(i): [] for i in range(len(testing_ft_models))}
            ...
            for model_name, response in prompts[prompt].items():
                ...
                if len(model_name) < 3:
                    if model_name in testing_ft_models.keys():
                        if int(gt_ft_model) == int(model_name):
                            labels.append(1)
                        else:
                            labels.append(0)
                        data_model_name.append(model_name)
    ...
    for model, predictions in model_correctness.items():
        ...
        output_data['test_results'][model] = {'accuracy': model_correctness[model] / num_model_samples,
                                                            'raw_correct':model_correctness[model],
                                                            'model_predictions': model_predictions[model],
    ...
    filename = f'./files/split_bert_{base_model}_classifier/test_results.json'
    with open(filename, 'w') as f:
       json.dump(output_data, f)
with open('./files/bert_split_classifier_table.tex', 'w') as f:
