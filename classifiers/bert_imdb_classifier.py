...
ft_models = {str(i): 'a' for i in range(10, 20)}
...
class BertClassifier(nn.Module):
    ...
    def save_model(self, base_model):
        self.bert.save_pretrained(f'../files/bert_imdb_unseen_classifier/bert_imdb_{base_model}_classifier')
        torch.save(self.linear.state_dict(), f'../files/bert_imdb_unseen_classifier/bert_imdb_{base_model}_classifier/ft_layer.pt')


    def load_model(self, base_model):
        self.bert.from_pretrained(f'../files/bert_imdb_{base_model}_classifier')
        self.linear.load_state_dict(torch.load(f'../files/bert_imdb_{base_model}_classifier/ft_layer.pt'))


def train(model, train_data, val_data, learning_rate, epochs, base_model, save):
...
with open('../files/ft_responses-imdb.json', 'r') as f:
...
for base_model in base_model_to_training_ft.keys():
        ...
        for prompt in prompts:
            ...
            for model_name, response in prompts[prompt].items():
                if base_model == model_name:
                    ...
                    data_model_name.append(model_name)
                if len(model_name) < 3:
                    if model_name in base_model_to_training_ft.values():
                        model_responses[model_name] = response[len(prompt):]
                        if gt_ft_model == model_name:
                            labels.append(1)
                        else:
                            labels.append(0)
                        data_model_name.append(model_name)
                        prompt_responses.append(prompts[prompt][base_model][len(prompt):] + '[SEP]' + response[len(prompt):])
    ...
    model = train(model, df_train, df_val, LR, EPOCHS, base_model, save=False)
    #model.load_model(base_model)
    labels = []
    prompt_responses = []
    data_model_name = []
        ...
        for prompt in prompts:
            ...
            for model_name, response in prompts[prompt].items():
                if base_model == model_name:
                    ...
                    data_model_name.append(model_name)
                if len(model_name) < 3:
                    if model_name in base_model_to_testing_ft.values():
                        model_responses[model_name] = response[len(prompt):]
                        if gt_ft_model == model_name:
                            labels.append(1)
                        else:
    ...
    filename = f'./files/bert_imdb_unseen_classifier/test_results_{base_model}.json'
    with open(filename, 'w') as f:
        json.dump(output_data, f)
with open('../files/bert_imdb_unseen_classifier/bert_imdb_unseen_classifier_table.tex', 'w') as f:
