...
for base_model in base_model_to_training_ft.keys():
    ...
    for dataset, prompts in training_responses.items():
        for prompt in prompts:
            ...
            labels.append(1)
            prompt_responses.append(prompts[prompt][base_model] + '[SEP]' + prompts[prompt][base_model])
            data_model_name.append(base_model)
            ft_key = random.choice(list(ft_models.keys()))
            prompt_responses.append(prompts[prompt][base_model] + '[SEP]' + prompts[prompt][ft_key])
            data_model_name.append(ft_key)
            if gt_ft_model == ft_key:
                labels.append(1)
            else:
                labels.append(0)
                            
    df = pd.DataFrame.from_dict({'labels': labels, 'response': prompt_responses, 'model': data_model_name})
    arr_3D = df.values.reshape(-1,2, df.shape[1])
    ...
    model_predictions = {str(i): []  for i in range(len(ft_models))}
    ...
    filename = f'../files/bert_bal_{base_model}_classifier/test_results.json'
    with open(filename, 'w') as f:
        json.dump(output_data, f)
with open('../files/bert_bal_classifier_table.tex', 'w') as f:
