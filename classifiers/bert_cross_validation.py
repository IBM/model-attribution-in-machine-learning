...
class BertClassifier(nn.Module):
    ...
    def save_model(self, base_model, test_models):
        print(f'../files/bert_{base_model}_{test_models[0]}_{test_models[1]}')
        self.bert.save_pretrained(f'../logs/bert_{base_model}_{test_models[0]}_{test_models[1]}')
        torch.save(self.linear.state_dict(), f'../logs/bert_{base_model}_{test_models[0]}_{test_models[1]}/ft_layer.pt')

    def load_model(self, base_model, test_models):
        self.bert.from_pretrained(f'../logs/bert_{base_model}_{test_models[0]}_{test_models[1]}')
        self.linear.load_state_dict(torch.load(f'../logs/bert_{base_model}_{test_models[0]}_{test_models[1]}/ft_layer.pt'))
...
for base_model in base_model_to_ft.keys():
    if 'codegen' not in base_model and 'xlnet' not in base_model:
        continue
    ...
    for test_set_pair in all_subsets(list(ft_models.keys())):
        ...
        torch.save(model.state_dict(), f'../logs/bert_{base_model}_{test_set_pair[0]}_{test_set_pair[1]}/model.pth')
        ...
        for model_test_name, predictions in model_correctness.items():
            ...
            if gt_ft_model == model_test_name:
                print(
                    f'ACCURACY OF {base_model}  SVM for {model_test_name} (GT): {model_correctness[model_test_name] / num_model_samples} ({model_correctness[model_test_name]}/{num_model_samples})', flush=True)
            else:
                print(
                    f'ACCURACY OF {base_model}  SVM for {model_test_name}: {model_correctness[model_test_name] / num_model_samples} ({model_correctness[model_test_name]}/{num_model_samples})', flush=True)
