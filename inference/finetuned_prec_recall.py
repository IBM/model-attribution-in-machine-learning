...
for base_model in base_model_to_ft.keys():
    ...
    for dataset in testing_datasets:
        ...
        model.load_state_dict(torch.load(f'./responses/{dataset}/finetuned_bert_base_ft_{base_model}_classifier/model.pth'))
        model.eval()
        labels = []
        prompt_responses = []
        data_model_name = []
...
with open('compute_responses/responses/pile_finetuned_classifications.tex', 'w') as f:
...
with open('compute_responses/responses/bert_pile_finetuned_pile.tex', 'w') as f:
    f.write(df_latex.to_latex())
