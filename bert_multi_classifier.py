...
for base_model in base_model_to_ft.keys():
            ...
            for model_name, response in prompts[prompt].items():
                ...
                if len(model_name) < 3:
                    if model_name in ft_models.keys():
                        ...
                        #    labels.append(int(model_name))
                        #else:
                        #    labels.append(0)
            model_as_string = ''
            shuffled_keys = list(model_responses.keys())[1:]
            random.shuffle(shuffled_keys)
            for key in shuffled_keys:
                model_as_string += f'\n {key}: {model_responses[key]} \n'
                if gt_ft_model == key:
                    labels.append(shuffled_keys.index(key))
                

            prompt_responses.append(model_as_string)
    ...
    for index, row in df_test.iterrows():
        ...
        with torch.no_grad():
            prediction = np.argmax(model(prompt_set['input_ids'], prompt_set['attention_mask']))
