...
class BertClassifier(nn.Module):
    ...
    def save_model(self, base_model):
        self.bert.save_pretrained(f'./files/bert_{base_model}_classifier')
        torch.save(self.linear.state_dict(), f'./files/bert_{base_model}_classifier/ft_layer.pt')
...
with open('./files/ft_responses-pile.json', 'r') as f:
    pile10k = json.load(f)
...
for base_model in base_model_to_ft.keys():
    ...
    for dataset in testing_datasets:
        ...
        model.load_state_dict(torch.load(f'./files/{dataset}/bert_base_ft_{base_model}_classifier/model.pth'))
        model.eval()
        labels = []
        prompt_responses = []
        data_model_name = []
...
with open('./files/pile_classifications_long10.tex', 'w') as f:
...
with open('./files/bert_pile_long10.tex', 'w') as f:
    f.write(df_latex.to_latex())
